...
export default function Home() {
  ...
  const contentInfo = () => {
    ...
    return <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {items.map((user) => {
  ...
  useEffect(() => {
    ...
    const newTotalPages = Math.ceil(1000 / 20) // Após testes, verificou-se que a api do github apenas permitir visualizar os 1000 primeiros resultados.
    if(newTotalPages === totalPages) return
    setTotalPages(newTotalPages)
  }, [total_count])

	return (
		<main className="mx-auto w-full max-w-360 flex flex-col gap-4">
